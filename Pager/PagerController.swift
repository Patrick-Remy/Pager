...
open class PagerController: UIViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate {
	...
	func defaultSetup() {
		...
		for i in 0 ..< self.tabCount {
            ...
            if tabLocation == .top, let nav = navigationController, nav.isNavigationBarHidden {
                frame.origin.y += UIApplication.shared.isStatusBarHidden ? 0.0 : 20.0
            }
		...
		if self.tabCount > 0 {
            ...
            if tabLocation == .top, let nav = navigationController, nav.isNavigationBarHidden {
                rect.origin.y += UIApplication.shared.isStatusBarHidden ? 0.0 : 20.0
            }
